...
if len(options.infilename.strip()) == 0:
    ...
    histName = 'bklmHists-'+ inputRoot +'-e{0}r{1}_odd.root'.format(exp, run)
    pdfName = 'bklmPlots-'+ inputRoot +'-e{0}r{1}_odd.pdf'.format(exp, run)
    eventPdfName = 'bklmEvents-'+ inputRoot +'-e{0}r{1}_odd.pdf'.format(exp, run)
    ...
    for row in infile.Get('KLM_raw_hits'):
        # stop processing after 20K events
        #if row.eventNr > 20000:
         #   break
        # process only the odd events (change to "== 1" for even events)
        if (row.eventNr % 2) == 0:
            continue
