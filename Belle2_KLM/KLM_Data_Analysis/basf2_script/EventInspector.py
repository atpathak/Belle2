...
class EventInspector(basf2.Module):
    ...
    def event(self):
        ...
        workingdigisector = [0,1,2,5,6]
        print (" length digits: ", len(digits))
        #print (" sector for end: ",digits[len(digits)-1].getSector())
        countdigi = 0
        counthit1d = 0
        counthit2d = 0
        
        for i in range(len(digits)):
            digisector = digits[i].getSector()-1
            if (digisector in workingdigisector):
                #print ("digisector: ", digisector)
                countdigi = countdigi + 1
        print("countdigi: ",countdigi)
        self.hist_nDigit.Fill(countdigi)
        
        
        #print (" length hit1d: ", len(hit1ds))
        for j in range(len(hit1ds)):
            hit1dsector = hit1ds[j].getSector()-1
            if (hit1dsector in workingdigisector):
                counthit1d = counthit1d + 1
        #print("counthit1d: ",counthit1d)
        self.hist_nHit1d.Fill(counthit1d)
        
        
        #print (" length hit2d: ", len(hit2ds))
        for k in range(len(hit2ds)):
            hit2dsector = hit2ds[k].getSector()-1
            if (hit2dsector in workingdigisector):
                counthit2d = counthit2d + 1
        #print("counthit2d: ",counthit2d)
        self.hist_nHit2d.Fill(counthit2d)
        ...
        rawStrip = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
        rawCtime = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
        workingdc = [0,1,2,3,9,11,12,13,14,15]
        ...
        workingsector = [0,1,2,5,6,8,9,10,13,14]
        for hit2d in hit2ds:
            ...
            if (sector in workingsector):
                sectorFB = sector if fb == 0 else sector + 8
            else:
                continue
            #print(" checking for hit", sectorFB)
            if layer >= 2:
                rpcHits[sectorFB] += 1
            if self.legacyTimes:
                dc = self.sectorFBToDC[sectorFB]
