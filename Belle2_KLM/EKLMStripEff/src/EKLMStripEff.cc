...
void EKLMStripEffModule::initialize()
{
  ...
  for(int cap=1;cap<=2;++cap)
	...
	{ char buff[100];
	  ...
	  if (cap == 2) {
	    sprintf(label, "occupancyForwardXY_Layer_%d", lay);
	    sprintf(title, "r"+runNumberTString+":Forward XY Occupancy for layer %d hits", lay);
	  } else {
	    sprintf(label, "occupancyBackwardXY_Layer_%d", lay);
	    sprintf(title, "r"+runNumberTString+":Backward XY Occupancy for layer %d hits", lay);
	  }
